...
% style
\tikzset{NNBox/.style={draw,color=OliveGreen,line width=2pt, fill=OliveGreen!20!white}}
...
\begin{tikzpicture}
    ...
    % text
    \node[empty, color=OliveGreen, scale=0.8] (NNtext) at ($(figNN)+(0,0.24*\ly)$) {Neural Network};
    \node[empty, color=OliveGreen, scale=0.8] (update) at ($(figNN)+(0.35*\lx,0.24*\ly)$) {Update the weights};
    \node[empty, color=OliveGreen, scale=0.8] (Optimization) at ($(figNN)+(0.65*\lx,0.1*\ly)$) {Optimization};
    ...
    \node[empty, yscale=8] (case) at ($(figNN)+(0.26*\lx,0*\ly)$) {$\}$};
    % arrows
    ...
    % Embedded Figures
    \node[empty] (fig1) at ($(SD1ll)!0.5!(SD1tr)+(0.015*\lx, 0.05*\ly)$) {\includegraphics[width=0.27\textwidth]{Figures/bed.pdf}};
    \node[empty] (fig2) at ($(SD2ll)!0.5!(SD2tr)+(0, 0.05*\ly)$) {\includegraphics[width=0.27\textwidth]{Figures/bed.pdf}};
    \node[empty] (fig3) at ($(SD3ll)!0.5!(Dtr)+(0, 0.05*\ly)$) {\includegraphics[width=0.27\textwidth]{Figures/bed.pdf}};
    % Equations
    \small
    \node[empty, scale=0.7] at ($(SD1ll)!0.5!(SD1tr)+(0.015*\lx, -0.16*\ly)$) {$\mathcal{L}_u=w_{{u}} \|\boldsymbol{u}-\boldsymbol{u}^{\text{obs}}\|_{\boldsymbol{u}}$};
    \node[empty, scale=0.7] at ($(SD2ll)!0.5!(SD2tr)+(0, -0.16*\ly)$) {$\mathcal{L}_g=w_{{g}}\left( \|{s}-{s}^{\text{obs}}\|_{{s}}+\|{H}-{H}^{\text{obs}}\|_{{H}}\right)$};
    \node[empty, scale=0.7] at ($(SD3ll)!0.5!(Dtr)+(0, -0.16*\ly)$) {$\mathcal{L}_C=w_{C} \|C-C^{\text{obs}}\|_{C}$};
    % arrows
    \draw[arrow, color=NavyBlue!90!white, to path={|- (\tikztotarget)}] ($(Lossmath)+(-0.06*\lx,0)$) --  +(0,-0.24*\ly) -| ($(fig1)+(0,0.14*\ly)$);
    \draw[arrow, color=NavyBlue!60!white, to path={|- (\tikztotarget)}] ($(Lossmath)+(0*\lx,0)$) --  +(0,-0.255*\ly) -| ($(fig2)+(0,0.14*\ly)$); 
    \draw[arrow, color=NavyBlue!30!white, to path={|- (\tikztotarget)}] ($(Lossmath)+(0.05*\lx,0)$) --  +(0,-0.27*\ly) -| ($(fig3)+(0,0.14*\ly)$);
    % text
    ...
    % Equations
    \node[empty, scale=0.7] (SSA) at ($(Pll)!0.5!(Ptr)+(0,0.05*\ly)$) {$
    \begin{cases}
        \begin{aligned}
    ...
    % arrow
    \draw[arrow, color=Orange!30!white, to path={|- (\tikztotarget)}] ($(Lossmath)+(0.1*\lx,0)$) --  +(0,-0.25*\ly) -| ($(Pll)!0.5!(Ptr)+(0,0.19*\ly)$);
    % text
