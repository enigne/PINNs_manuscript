...
\begin{tikzpicture}
    ...
    % Equations
    \small
    \node[empty, scale=0.7] at ($(SD1ll)!0.5!(SD1tr)+(0.015*\lx, -0.14*\ly)$) {$\mathcal{L}_u=w_{{u}} \|\boldsymbol{u}-\boldsymbol{u}^{\text{obs}}\|_{\boldsymbol{u}}$};
    \node[empty, scale=0.7] at ($(SD2ll)!0.5!(SD2tr)+(0, -0.14*\ly)$) {$\mathcal{L}_g=w_{{g}}\left( \|{s}-{s}^{\text{obs}}\|_{{s}}+\|{H}-{H}^{\text{obs}}\|_{{H}}\right)$};
    \node[empty, scale=0.7] at ($(SD3ll)!0.5!(Dtr)+(0, -0.14*\ly)$) {$\mathcal{L}_C=w_{C} \|C-C^{\text{obs}}\|_{C}$};
    % arrows
